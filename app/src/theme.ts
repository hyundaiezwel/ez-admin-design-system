...
/**
 * 다크용 surface 램프 — **한 칸 밝은 쪽으로 민다.**
 *
 * Aura는 같은 `{surface.N}` 인덱스를 배경에도 글자에도 쓴다. 라이트 램프를 그대로 물리면
 * 중간 단계(`{surface.500}`)가 어두운 배경 위 글자색으로 쓰여 대비가 무너진다 —
 * 보조 버튼 글자가 3.69:1까지 떨어졌다(실측). 배경으로 쓰이는 900/950은 그대로 두고
 * 중간만 민다.
 */
const ezSurfaceDark = {
  0: 'var(--ez-color-gray-0)',
  ...
  200: 'var(--ez-color-gray-20)',
  300: 'var(--ez-color-gray-30)',
  400: 'var(--ez-color-gray-30)',
  500: 'var(--ez-color-gray-40)',
  600: 'var(--ez-color-gray-50)',
  700: 'var(--ez-color-gray-70)',
  800: 'var(--ez-color-gray-80)',
  900: 'var(--ez-color-gray-90)',
  950: 'var(--ez-color-gray-95)',
...
export const EzPreset = definePreset(Aura, {
  ...
  semantic: {
    ...
    formField: {
      // 우리 규격은 sm 28 / md 34 / lg 40. Aura 기본은 더 넉넉하다.
      // 스파이크에서 padding만 줄여 38px까지 내렸고, 남은 4px은 글자 크기였다 —
      // PrimeVue 기본이 1rem(16px)인데 우리 본문은 13px이다.
      fontSize: 'var(--ez-font-size-sm)',
